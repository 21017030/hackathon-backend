...
def _rewrite_query(content: str, history: list) -> str:
    ...
    prompt = f"""아래 대화 내역을 참고하여 사용자의 현재 질문에서 빠진 맥락(지시어, 생략된 주어 등)만 보완하세요.

규칙:
- 원래 질문의 의도와 표현을 최대한 유지하세요.
- '그거', '이거', '거기서', '그 부분' 같은 지시어가 있을 때만 구체적인 내용으로 교체하세요.
- 질문이 이미 명확하다면 그대로 반환하세요.
- 강의자료 커버리지나 설명 정도에 대한 질문('자세히 안나와있지 않아?', '내용이 없어?' 등)은 절대 검색 쿼리로 바꾸지 마세요.
- 변환된 쿼리만 한 줄로 출력하세요.
